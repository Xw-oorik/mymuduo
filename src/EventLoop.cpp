...
// 创建wakeupFd，用来通知唤醒subreactor处理新channel
int createEventfd()
{
...
{
    quit_ = true;
    // 下面为了处理，如果是在其他线程中，调用quit在一个subloop(woker)中，调用了mainLoop(IO)的quit
    if (!isInLoopThread())
    {
        wakeup();
    }
...
void EventLoop::queueInLoop(Functor cb)
{
    ...
    // || callingPendingFunctors_的意思是：当前loop正在执行回调，但是loop又有了新的回调需要操作
